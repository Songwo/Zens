...
public class RecommendController {
    ...
    @GetMapping("/list")
    public Result<IPage<PostRecommendResp>> getRecommendList(
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int pageSize) {
        
        String userId = SecurityUtils.getCurrentUserId();
        IPage<PostRecommendResp> recommendations = postRecommendService.getHybridRecommendations(userId, page, pageSize);
        return Result.success(recommendations);
    }

    @Operation(summary = "获取帖子详情页底部推荐", description = "同专业发布、相同分类标签、热度兜底")
    @GetMapping("/post-detail/{postId}")
    public Result<List<PostRecommendResp>> getPostDetailRecommend(
