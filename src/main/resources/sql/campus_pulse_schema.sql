...
-- ============================================================
-- 14.1) 隐私友好的站内增长事件（不存 IP、UA、正文或凭据）
-- ============================================================
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='站内产品增长事件';
...
-- 28) 自动运营内容计划表
-- ============================================================
CREATE TABLE `ops_content_plan` (
  `id`              varchar(64)   NOT NULL,
  `idempotency_key` varchar(128)  NOT NULL COMMENT '创建计划幂等键',
  ...
  `created_by`      varchar(64)   NOT NULL COMMENT '服务身份或人工操作者',
  `create_time`     datetime      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time`     datetime      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ops_plan_idem` (`idempotency_key`),
  KEY `idx_ops_plan_status_schedule` (`status`, `scheduled_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='自动运营内容计划表';

-- ============================================================
-- 29) 自动运营草稿表
-- ============================================================
CREATE TABLE `ops_draft` (
  `id`                varchar(64)    NOT NULL,
  `idempotency_key`   varchar(128)   NOT NULL COMMENT '创建草稿幂等键',
  `plan_id`           varchar(64)    DEFAULT NULL,
  `type`              varchar(20)    NOT NULL COMMENT 'POST/COMMENT',
  ...
  `update_time`       datetime       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ops_draft_idem` (`idempotency_key`),
  KEY `idx_ops_draft_status_time` (`status`, `create_time`),
  KEY `idx_ops_draft_target` (`target_post_id`),
  KEY `idx_ops_draft_plan` (`plan_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='自动运营帖子与评论草稿表';

-- ============================================================
-- 30) 自动运营审批记录表
-- ============================================================
CREATE TABLE `ops_approval` (
  `id`          varchar(64)   NOT NULL,
  `draft_id`    varchar(64)   NOT NULL,
  `action`      varchar(20)   NOT NULL COMMENT 'APPROVE/REJECT/PUBLISH',
  `operator_id` varchar(64)   NOT NULL,
  `note`        varchar(500)  DEFAULT NULL,
  `create_time` datetime      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_ops_approval_draft_time` (`draft_id`, `create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='自动运营人工审批记录表';
...
CREATE TABLE `ops_job_run` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ops_job_idem` (`idempotency_key`),
  KEY `idx_ops_job_type_time` (`job_type`, `started_at`),
  KEY `idx_ops_job_status_time` (`status`, `started_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='自动运营任务执行与失败审计表';
...
CREATE TABLE `ops_metric_snapshot` (
  ...
  `create_time`     datetime      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ops_metric_idem` (`idempotency_key`),
  KEY `idx_ops_metric_period` (`period_start`, `period_end`)
...
CREATE TABLE `supporter_plan` (
  ...
  `description` varchar(500) NOT NULL, `price_cents` int NOT NULL, `currency` char(3) NOT NULL DEFAULT 'CNY',
  `duration_days` int NOT NULL, `benefits_json` varchar(2000) NOT NULL, `active` tinyint(1) NOT NULL DEFAULT 1,
  `sort_weight` int NOT NULL DEFAULT 0, `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`), UNIQUE KEY `uk_supporter_plan_code` (`code`), KEY `idx_supporter_plan_active_sort` (`active`, `sort_weight`)
...
CREATE TABLE `payment_callback_event` (
  `id` bigint NOT NULL AUTO_INCREMENT, `provider` varchar(30) NOT NULL, `event_id` varchar(160) NOT NULL,
  `order_no` varchar(40) DEFAULT NULL, `payload_hash` char(64) NOT NULL, `signature_valid` tinyint(1) NOT NULL,
  `status` varchar(20) NOT NULL, `error_message` varchar(500) DEFAULT NULL,
  `received_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, `processed_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`), UNIQUE KEY `uk_payment_callback_provider_event` (`provider`, `event_id`),
  KEY `idx_payment_callback_order` (`order_no`, `received_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='支付回调幂等与审计账本';

CREATE TABLE `supporter_entitlement` (
  `id` bigint NOT NULL AUTO_INCREMENT, `user_id` varchar(64) NOT NULL, `plan_code` varchar(50) NOT NULL,
  `plan_name_snapshot` varchar(80) NOT NULL, `source_order_no` varchar(40) NOT NULL,
  `starts_at` datetime NOT NULL, `expires_at` datetime NOT NULL, `status` varchar(20) NOT NULL DEFAULT 'ACTIVE',
