...
import { Clock, EditPen, Warning, Delete, View } from '@element-plus/icons-vue'
import type { Post } from '@/types'

const router = useRouter()
const userStore = useUserStore()
const uid = () => userStore.userId || userStore.userInfo?.id || ''
const sub = ref<'drafts' | 'pending' | 'rejected' | 'trash' | 'history'>('drafts')
const options = [
...
const pendingFetcher = (page: number) => postApi.searchList({ page, pageSize: 10, needTotal: false, userId: uid(), status: 1, auditStatus: 'PENDING' }).then(r => r.data.records as Post[])
...
<template>
  <div class="creator-panel">
    ...
    <div class="creator-body">
      ...
      <ProfilePostList v-else-if="sub === 'pending'" :fetcher="pendingFetcher" :empty-icon="Clock" empty-title="暂无待审核帖子" empty-description="提交后等待版务确认的帖子会在这里" />
