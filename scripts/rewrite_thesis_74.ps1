param(
    [string]$SourceDoc = "docs\赵青松论文7.4.docx",
    [string]$OutputDoc = "docs\赵青松论文7.4_按要求改写.docx"
)
...
$outputPath = Join-Path $repoRoot $OutputDoc
$diagramDir = Join-Path $repoRoot "tmp_thesis74\generated_diagrams"

if (-not (Test-Path $sourcePath)) {
    throw "未找到源论文文件: $sourcePath"
}

& (Join-Path $PSScriptRoot 'generate_thesis_diagrams.ps1') -OutputDir "tmp_thesis74\generated_diagrams" | Out-Null
...
function Replace-ParagraphText {
    ...
    $Document.Paragraphs.Item($ParagraphIndex).Range.Text = Convert-ToWordText $Text
}

function Insert-PictureAtParagraph {
    param(
        $Document,
        [int]$ParagraphIndex,
        [string]$PicturePath,
...
$chapter3Intro = @"
经过对技术社区类平台的需求分析，系统功能结构不再按英文缩写或技术名词堆砌，而是按照真实业务角色与业务场景组织。普通用户模块覆盖注册登录、发帖存草稿、评论互动、收藏关注、私信通知、个人中心与推荐浏览；版主模块覆盖我的板块、帖子审核、举报处理、板块置顶和精华设置；管理员模块按照后台导航栏划分为数据看板、内容管理、板块管理、用户管理、举报管理、缓存管理、发展历程管理、版主申请管理和邀请码管理；数据分析层则负责热度排行、关键词聚合、发帖趋势与推荐分发。由此形成“前台内容生产 + 分区治理 + 后台运维 + 趋势决策”的整体功能结构，如图3—1所示。
"@

$chapter3DatabaseIntro = @"
数据库系统的概念结构设计旨在抽象反映现实业务对象及其联系。为便于论文表达并避免直接使用英文表名和字段缩写，图3—2至图3—19中的实体名称、属性名称和联系名称均采用中文业务语义描述。本系统围绕“用户、内容、互动、治理、统计推荐”五大业务域展开，覆盖账号认证、社区发帖、消息通知、内容审核与热度分析等核心功能。
"@

$chapter3DatabaseScope = @"
结合项目实际数据库结构，本节选取用户、标签、板块、帖子、帖子点赞、帖子收藏、评论、浏览日志、等级经验日志、通知、私信、举报、关注关系、版主申请、评论点赞、热度快照和趋势统计 17 个核心实体进行说明；邀请码、发展历程及用户标签关联等辅助表在逻辑结构设计部分补充说明。
"@
...
$figureParagraphs = @(
    @{ Paragraph = 169; File = 'fig3-1-system-function.png'; Width = 460 },
    @{ Paragraph = 176; File = 'fig3-2-user.png'; Width = 430 },
    @{ Paragraph = 179; File = 'fig3-3-tag.png'; Width = 430 },
    @{ Paragraph = 182; File = 'fig3-4-section.png'; Width = 430 },
    @{ Paragraph = 185; File = 'fig3-5-post.png'; Width = 440 },
    @{ Paragraph = 188; File = 'fig3-6-post-like.png'; Width = 420 },
    @{ Paragraph = 191; File = 'fig3-7-post-collect.png'; Width = 420 },
    @{ Paragraph = 194; File = 'fig3-8-comment.png'; Width = 430 },
    @{ Paragraph = 197; File = 'fig3-9-view-log.png'; Width = 430 },
    @{ Paragraph = 200; File = 'fig3-10-level-log.png'; Width = 420 },
    @{ Paragraph = 203; File = 'fig3-11-notification.png'; Width = 430 },
    @{ Paragraph = 206; File = 'fig3-12-message.png'; Width = 430 },
    @{ Paragraph = 209; File = 'fig3-13-report.png'; Width = 430 },
    @{ Paragraph = 212; File = 'fig3-14-follow.png'; Width = 420 },
    @{ Paragraph = 215; File = 'fig3-15-moderator-application.png'; Width = 430 },
    @{ Paragraph = 218; File = 'fig3-16-comment-like.png'; Width = 420 },
    @{ Paragraph = 221; File = 'fig3-17-heat-snapshot.png'; Width = 420 },
    @{ Paragraph = 224; File = 'fig3-18-trend-stat.png'; Width = 430 },
    @{ Paragraph = 227; File = 'fig3-19-overall-er.png'; Width = 500 }
)
...
try {
    ...
    Replace-ParagraphText -Document $document -ParagraphIndex 167 -Text $chapter3Intro
    Replace-ParagraphText -Document $document -ParagraphIndex 173 -Text $chapter3DatabaseIntro
    Replace-ParagraphText -Document $document -ParagraphIndex 174 -Text $chapter3DatabaseScope

    foreach ($figure in $figureParagraphs) {
        Insert-PictureAtParagraph -Document $document `
            -ParagraphIndex $figure.Paragraph `
            -PicturePath (Join-Path $diagramDir $figure.File) `
            -Width $figure.Width
    }
    $document.Save()
    ...
    [void](Add-Table -Document $document -Selection $selection -Rows $table51)
    ...
    [void](Add-Table -Document $document -Selection $selection -Rows $table52)
    ...
    [void](Add-Table -Document $document -Selection $selection -Rows $table53)
    ...
    Write-Output "Rewritten thesis saved to $outputPath"
} finally {
    if ($document -ne $null) {
        $document.Close([ref]0)
    }
